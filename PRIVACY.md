...
server(s) **you** configure in Settings (any server that speaks OPDS,
KOSync, or the Kobo sync protocol). The developer does not operate any
backend.
...
- Book files (`.epub`) downloaded from your server — stored in the
  app's sandbox.
...
1. **The server URL(s) you enter in Settings**, for:
...
Your server has its own privacy policy and operator. Kios is not
